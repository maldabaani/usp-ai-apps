...
to a manual one. This is also the single place a future ingestion-generation
cache-invalidation counter bump (see plan file section L, Phase F) belongs,
in each function's success branch only -- not the except branch, since a
failed run didn't actually change the corpus.
"""
from __future__ import annotations

from api.ingest_jobs import fail_job, finish_job, update_progress
from ingestion import ingest_job_registry
