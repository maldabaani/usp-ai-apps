...
Ported from com.jslogicextractor.agent.ClaudeLogicExtractionAgent. Rebuilds
its ChatAnthropic client only when settings.settings_generation has advanced
(the same generation-counter pattern pipeline/nodes/generate.py's _get_llm()
and clarify.py use, replacing Java's RuntimeSettings.generation() +
rebuildIfNeeded()) -- so a settings-screen change to the Anthropic key/model
takes effect on the next extraction without a restart. Single attempt per
file, catching all exceptions into a failure_result (no internal retry loop,
matching Java's actual per-file-extraction behavior).
"""
