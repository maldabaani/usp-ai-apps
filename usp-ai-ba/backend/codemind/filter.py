"""Cheap, regex-only pre-pass applied to every file regardless of execution
mode (sync or batch): skips files that would burn an LLM call on output with
no real business logic to extract. Deliberately conservative -- a false
negative just costs one wasted call, but a false positive silently drops a
file's extraction entirely, so each rule only matches unambiguous cases.

Ported from com.jslogicextractor.filter.NonSubstantiveFileFilter.
"""
