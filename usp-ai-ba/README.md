...
backend/
  ...
  ingestion/
    ...
    ingest_documents.py        PDF/Word (.docx) chunking + embedding into sf_user_manuals, plus an optional per-document LLM-summary enrichment tier (enrichment/enrich_documents.py)
    ingest_code.py             Mechanical structural chunking (16 languages) + embedding into sf_codebase/sf_jpa_entities, incremental-skip via manifest.py, plus an optional per-file LLM-summary enrichment tier (enrichment/enrich.py)
    manifest.py                Per-repo content-hash manifest (compute_hash/load/save/diff), shared by ingest_code.py's own chunking-skip and both enrichment tiers' summarization-skip -- three independent manifest namespaces (tier 1 chunking, tier 2 code enrichment, tier 2 document enrichment)
    enrichment/                Optional per-file LLM-summary enrichment tier for both code (enrich.py, prompts.py) and documents (enrich_documents.py, doc_prompts.py), sharing one set of Claude/Ollama agents (agents/) via an injectable prompt-builder override
...
| `INGEST_OLLAMA_ENABLED` | `true` | Whether Ollama is available at all for ingestion's optional per-file LLM-summary enrichment tier (on by default). Only relevant when `INGEST_LLM_MODEL=ollama` below — when `INGEST_LLM_MODEL=claude`, Ollama is always available as the fallback regardless of this flag |
| `INGEST_OLLAMA_MODEL` | `qwen2.5:14b` | Model name for the above |
| `INGEST_LLM_MODEL` | `ollama` | `ollama` (default) or `claude` — which model is the PRIMARY choice for a whole ingestion run's LLM-summary enrichment tier. Replaces the old behavior of registering both Claude and Ollama simultaneously and round-robinning between them per file/part regardless of health. When set to `claude`, Ollama is always available as a sticky fallback: once Claude fails once (e.g. exhausted API credits), every remaining file/part in that same run uses Ollama instead, with no further Claude attempts — the next ingestion run retries Claude fresh. Editable from `/settings` (takes effect on the next ingestion run, no restart needed) |
...
| `JOBS_DIR` | `./jobs` | Holds `checkpoints.sqlite` (LangGraph's persisted pipeline state), `assess_jobs.json` (the dashboard's job list), `users.json`, `.jwt_secret`, `.chunking-manifests/` (tier 1's incremental-skip manifests), `.enrichment-manifests/` (tier 2 code enrichment's incremental-skip manifests), and `.document-enrichment-manifests/` (tier 2 document enrichment's incremental-skip manifests) — all survive a backend restart |
...
curl -X POST http://localhost:8000/api/ingest/documents -H "Content-Type: application/json" \
  -d '{"folder_path": "/path/to/user-manuals", "enable_llm_summary": true, "max_concurrency": 8}'

curl -X POST http://localhost:8000/api/ingest/code -H "Content-Type: application/json" \
  -d '{"repo_path": "/path/to/monorepo"}'
```

Both return `{"job_id": ..., "status": "pending"}` immediately; poll `GET /api/ingest/status/{job_id}` for progress/completion, or use the `/ingestion` page. Re-running ingestion against the same repo/folder **updates in place**: for code ingestion, a per-repo content-hash manifest (`JOBS_DIR/.chunking-manifests/`) means a file whose content hasn't changed since the last successful run is skipped entirely (no re-chunking, no re-embedding), not just harmlessly re-upserted — changed files are re-chunked and their stale chunks replaced, and files deleted since the last run have their chunks purged (independent of the manifest, via a diff against what's already in Chroma). Pass `"force_full_rechunk": true` in the `POST /api/ingest/code` body (or the checkbox on `/ingestion`) to bypass the skip for one run only, e.g. if the manifest is ever suspected stale — the manifest itself isn't disabled, so the next normal run goes back to skipping unchanged files.
...
**Per-file success/skip breakdown**: both the active job's live progress and each History row on the `/ingestion` page expand into a filterable, searchable per-file table (grouped success/skipped/error counts) covering both tiers — tier 1's mechanical chunking (`success`/`skipped` with reason `unchanged_since_last_run`/`error`) and tier 2's LLM-summary enrichment (`summarized`/`skipped` with a specific reason/`error`), so a credit-exhaustion-style enrichment failure is no longer indistinguishable from a deliberate skip (a test/spec file, an unchanged-since-last-run file, enrichment disabled, or no agent configured).
...
- A file whose content hash matches its last successfully-chunked run is skipped entirely (no re-chunking, no re-embedding) via a per-repo manifest under `JOBS_DIR/.chunking-manifests/` (`backend/ingestion/manifest.py`, shared with tier 2 below but keyed under a separate namespace, since "chunked" and "summarized" are independent done-states). A file's hash is only recorded once its chunking+embedding genuinely succeeds, so a transient failure is retried on the next run rather than silently treated as done. A skipped file's existing chunks (including its `sf_jpa_entities` entry, if any) are left untouched.

**Optional LLM-summary enrichment** (`backend/ingestion/enrichment/`, `INGEST_LLM_SUMMARY_ENABLED`, on by default): after mechanical chunking, each eligible file is also sent to an LLM (Ollama by default, optionally Claude too) for a narrative summary, embedded alongside the raw chunks. For code (`enrich.py`), this is a business-logic summary written into `sf_codebase`. For documents (`enrich_documents.py`), the same Claude/Ollama agent classes are pointed at a document-oriented prompt (`doc_prompts.py`, extracting business rules/policies rather than code logic) and the summary is written into `sf_user_manuals` instead — meant to give StoryForge's assessment and Ask Technical/Business richer, LLM-synthesized context from manuals, not just raw mechanically-split text. Incremental re-runs skip re-summarizing files whose content hasn't changed, via a per-repo content-hash manifest — code under `JOBS_DIR/.enrichment-manifests/`, documents under their own independent `JOBS_DIR/.document-enrichment-manifests/` namespace. Unlike code, documents have no `force_full_rechunk`-equivalent override — a stale/stuck document summary can only be refreshed by an actual content change to the source file.
...
| `POST` | `/api/ingest/documents` | Body: `{"folder_path": str, "enable_llm_summary"?: bool, "max_concurrency"?: int}`. Starts background document ingestion (PDF/Word (.docx) only) → `{"job_id", "status": "pending"}` |
| `POST` | `/api/ingest/code` | Body: `{"repo_path": str, "enable_llm_summary"?: bool, "max_concurrency"?: int, "force_full_rechunk"?: bool}`. Starts background code ingestion → `{"job_id", "status": "pending"}` |
...
| `POST` | `/api/corpus/sources/delete` | Admin only. Body: `{"collection_key": "manuals"\|"codebase", "source": str}`. Removes that source's chunks (idempotent — deleting an already-gone source isn't an error); deleting a `codebase` source also clears its `entities` chunks, if any. Does **not** clear either tier's content-hash manifest — if the source file still exists on disk with unchanged content, a later full re-ingestion of its parent repo skips it in both tiers (neither the mechanical chunks nor the LLM summary come back) unless the file's content actually changes or (for code only) that run passes `force_full_rechunk: true` — documents have no equivalent override, so a deleted-but-unchanged manual can only come back via an actual content change |
...
- **Ingestion completes but the LLM-summary enrichment tier silently did nothing** — check `GET /api/ingest/status/{job_id}`'s `result.errors` list, or the `/monitoring` page, for `"credit balance too low"`/`"invalid x-api-key"`/similar per-file failures. If `INGEST_LLM_MODEL=claude` and Claude fails, the rest of that run automatically falls back to Ollama (see `INGEST_LLM_MODEL` above) — a run-wide failure with no fallback used only happens if `INGEST_LLM_MODEL=ollama` and `INGEST_OLLAMA_ENABLED` is also off. Mechanical chunking (tier 1) still succeeds regardless; only the optional narrative summaries are missing. Set a valid key/model and re-run ingestion — a failed enrichment attempt is never recorded as done, so it will be retried automatically.
