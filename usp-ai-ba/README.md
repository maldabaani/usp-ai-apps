...
backend/
  ...
  ingestion/
    ...
    ingest_code.py             Mechanical structural chunking (16 languages) + embedding into sf_codebase/sf_jpa_entities, incremental-skip via manifest.py
    manifest.py                Per-repo content-hash manifest (compute_hash/load/save/diff), shared by ingest_code.py's own chunking-skip and enrichment/'s summarization-skip -- separate manifest namespaces per tier
    enrichment/                Optional per-file LLM-summary enrichment tier (agents/, prompts.py), incremental skip via ../manifest.py
...
| `JOBS_DIR` | `./jobs` | Holds `checkpoints.sqlite` (LangGraph's persisted pipeline state), `assess_jobs.json` (the dashboard's job list), `users.json`, `.jwt_secret`, `.chunking-manifests/` (tier 1's incremental-skip manifests), and `.enrichment-manifests/` (tier 2's incremental-skip manifests) — all survive a backend restart |
